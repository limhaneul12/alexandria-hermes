...
# Broad type justified: Pydantic before validators receive raw boundary input.
def parse_scope_value(value: object) -> ContextScope:
    """Parse public JSON context scope values.
    ...
    """
    if isinstance(value, ContextScope):
        return value
    if isinstance(value, str):
        return ContextScope(value)
    raise ValueError("scope must be a valid context scope")
