...
async def capture_harness(
    ...
    """
    context = await service.capture_harness(_harness_capture(request))
    response = ContextResponse.model_validate(context_payload(context))
    return response


def _harness_capture(request: HarnessCaptureRequest) -> HarnessCapture:
...
@router.post(
    ...
    status_code=status.HTTP_200_OK,
    description="Validate generated HARNESS content without saving it.",
    summary="Check execution harness",
)
@router_exception_status(CONTEXT_ROUTE_EXCEPTION_MAPPING)
@inject
async def check_harness(
    ...
    """Check an agent-owned execution harness without persistence.
    ...
    Returns:
        Context lint response for the generated HARNESS content.
    """
    result = service.check_harness(_harness_capture(request))
...
@router.get(
    ...
    response_model=ContextListResponse,
    status_code=status.HTTP_200_OK,
    description="List saved execution harness contexts.",
    summary="List execution harnesses",
)
@router_exception_status(CONTEXT_ROUTE_EXCEPTION_MAPPING)
@inject
async def list_harnesses(
    ...
    Returns:
        Paginated HARNESS context response.
    """
    items, total = await service.list_harnesses(
        limit=limit,
        offset=offset,
        project=project,
        scope=scope,
...
@router.get(
    "/harnesses/{context_id}",
    response_model=ContextResponse,
    status_code=status.HTTP_200_OK,
    description="Read one saved execution harness context.",
    summary="Get execution harness",
)
@router_exception_status(CONTEXT_ROUTE_EXCEPTION_MAPPING)
@inject
async def get_harness(
    ...
    """Get one stored HARNESS context.
    ...
    Returns:
        Stored HARNESS context response.
    """
    context = await service.get_harness(context_id)
    response = ContextResponse.model_validate(context_payload(context))
    return response


@router.post(
    "/harnesses/{context_id}/archive",
    response_model=ContextResponse,
    status_code=status.HTTP_200_OK,
    description="Archive one execution harness context.",
    summary="Archive execution harness",
)
@router_exception_status(CONTEXT_ROUTE_EXCEPTION_MAPPING)
@inject
async def archive_harness(
    ...
    """Archive one stored HARNESS context.
    ...
    Returns:
        Archived HARNESS context response.
    """
    context = await service.archive_harness(context_id)
    response = ContextResponse.model_validate(context_payload(context))
    return response
