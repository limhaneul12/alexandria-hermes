import { PrismaClient } from "@prisma/client";

const prisma = new PrismaClient();

const categories = [
  { name: "Computer Science", slug: "computer-science", parentSlug: null },
  { name: "Programming", slug: "programming", parentSlug: "computer-science" },
  { name: "Python", slug: "python", parentSlug: "programming" },
  { name: "FastAPI", slug: "fastapi", parentSlug: "python" },
  { name: "DevOps", slug: "devops", parentSlug: "computer-science" },
  { name: "System Design", slug: "system-design", parentSlug: "computer-science" },
];

const skills = [
  {
    title: "FastAPI Dependency Injection",
    slug: "fastapi-dependency-injection",
    description: "A production pattern for composing FastAPI dependencies, request-scoped sessions, and domain services.",
    content: "Use explicit dependency helpers for request-scoped resources, keep providers async-safe, and route use cases through narrow ports.\n\nUsage Guide:\n1. Define domain ports.\n2. Compose repositories in infrastructure.\n3. Expose async FastAPI dependency helpers.\n\n```py\nasync def get_service(session = Depends(get_db_session)):\n    return Service(repo=Repository(session=session))\n```",
    type: "SKILL",
    version: "2.1.0",
    author: "Hermes Librarian",
    categorySlug: "fastapi",
    tags: ["FastAPI", "DI", "Python"],
  },
  {
    title: "Redis Streams Consumer Recovery",
    slug: "redis-streams-consumer-recovery",
    description: "Recovery playbook for stalled Redis stream consumers, pending entries, and safe replay windows.",
    content: "Inspect pending entries, claim orphaned messages, and apply retry/drop policy with explicit idempotency keys.\n\n```bash\nXINFO GROUPS events\nXPENDING events workers\n```",
    type: "WORKFLOW",
    version: "1.4.3",
    author: "Operations Scribe",
    categorySlug: "devops",
    tags: ["Redis", "Streams", "Recovery"],
  },
  {
    title: "AsyncIO Event Loop Deep Dive",
    slug: "asyncio-event-loop-deep-dive",
    description: "Deep operating notes for event loop scheduling, task cancellation, shielding, and backpressure.",
    content: "Treat cancellation as a first-class control flow signal. Use bounded queues and explicit timeouts at IO edges.\n\n```py\nasync with asyncio.timeout(3):\n    await client.fetch()\n```",
    type: "KNOWLEDGE",
    version: "3.0.0",
    author: "Alexandria Core",
    categorySlug: "python",
    tags: ["AsyncIO", "Python", "Concurrency"],
  },
  {
    title: "Celery Retry Strategy",
    slug: "celery-retry-strategy",
    description: "Retry, backoff, dead-letter, and idempotency conventions for resilient task processing.",
    content: "Separate transient failures from permanent domain failures. Use exponential backoff and bounded retry counts.\n\n```py\nraise self.retry(exc=exc, countdown=backoff)\n```",
    type: "WORKFLOW",
    version: "1.8.1",
    author: "Hermes Librarian",
    categorySlug: "devops",
    tags: ["Celery", "Retries", "Queues"],
  },
  {
    title: "SQLAlchemy Session Management",
    slug: "sqlalchemy-session-management",
    description: "Session lifecycle rules for async SQLAlchemy repositories and request-scoped transactions.",
    content: "Create sessions at request boundaries, commit once per successful request, rollback on failure, and keep ORM models inside infrastructure.\n\n```py\nasync with sessionmaker() as session:\n    yield session\n```",
    type: "SKILL",
    version: "2.0.2",
    author: "Database Archivist",
    categorySlug: "python",
    tags: ["SQLAlchemy", "Database", "Python"],
  },
  {
    title: "API Contract Review Checklist",
    slug: "api-contract-review-checklist",
    description: "A compact review workflow for schema examples, response stability, errors, and pagination contracts.",
    content: "Check explicit schema examples, stable error naming, cursor behavior for scroll surfaces, and endpoint descriptions.",
    type: "CHECKLIST",
    version: "1.2.0",
    author: "Grand Archive Council",
    categorySlug: "system-design",
    tags: ["API", "Review", "Design"],
  },
];

const workflows = [
  {
    title: "Backend Rule Compliance Review",
    slug: "backend-rule-compliance-review",
    description: "A repeatable archive workflow for checking FastAPI code against local engineering rules.",
    content: "Inspect rule docs, map changed files, run lint/type/tests, and produce severity-rated findings.",
    author: "Hermes Librarian",
  },
  {
    title: "Capability Intake Triage",
    slug: "capability-intake-triage",
    description: "Classify incoming knowledge into skill, workflow, or document before indexing.",
    content: "Normalize input, assign category, tag by operational domain, and capture usage intent.",
    author: "Archive Curator",
  },
];

const knowledgeDocuments = [
  {
    title: "Dependency Injection Boundary Notes",
    slug: "dependency-injection-boundary-notes",
    description: "Design notes on root containers, bounded-context containers, and async-safe FastAPI wiring.",
    content: "Root containers own shared resources; bounded contexts compose local use cases and repositories.",
    categorySlug: "fastapi",
  },
  {
    title: "Operational Pagination Policy",
    slug: "operational-pagination-policy",
    description: "When to use limit/offset, cursor-based pagination, and infinite scroll contracts.",
    content: "Use cursor pagination for scrollable changing lists and limit/offset for small admin views.",
    categorySlug: "system-design",
  },
];

const recommendations = [
  {
    title: "Review FastAPI DI before adding routers",
    description: "Most agent failures come from unclear dependency boundaries and request-session ownership.",
    type: "SKILL",
    usageCount: 128,
  },
  {
    title: "Pair search APIs with FTS-safe query normalization",
    description: "Prevent operator syntax from surfacing as 500s in library search.",
    type: "KNOWLEDGE",
    usageCount: 91,
  },
  {
    title: "Use cursor pagination for scroll-heavy archives",
    description: "Preserves stable exploration as the archive grows and changes.",
    type: "POLICY",
    usageCount: 72,
  },
];

async function seed() {
  await prisma.usageHistory.deleteMany();
  await prisma.recommendation.deleteMany();
  await prisma.skill.deleteMany();
  await prisma.workflow.deleteMany();
  await prisma.knowledgeDocument.deleteMany();
  await prisma.tag.deleteMany();
  await prisma.category.deleteMany();
  await prisma.$executeRawUnsafe(
    "DELETE FROM sqlite_sequence WHERE name IN ('Category', 'Tag', 'Skill', 'Workflow', 'KnowledgeDocument', 'UsageHistory', 'Recommendation')",
  );

  const categoryBySlug = new Map();
  for (const category of categories) {
    const created = await prisma.category.create({
      data: {
        name: category.name,
        slug: category.slug,
        parentId: category.parentSlug ? categoryBySlug.get(category.parentSlug).id : null,
      },
    });
    categoryBySlug.set(category.slug, created);
  }

  const tagByName = new Map();
  for (const skill of skills) {
    for (const tagName of skill.tags) {
      if (!tagByName.has(tagName)) {
        const tag = await prisma.tag.create({ data: { name: tagName } });
        tagByName.set(tagName, tag);
      }
    }
  }

  const now = Date.now();
  for (let index = 0; index < skills.length; index += 1) {
    const skill = skills[index];
    const created = await prisma.skill.create({
      data: {
        title: skill.title,
        slug: skill.slug,
        description: skill.description,
        content: skill.content,
        type: skill.type,
        version: skill.version,
        author: skill.author,
        categoryId: categoryBySlug.get(skill.categorySlug).id,
        lastAccessedAt: new Date(now - (index + 1) * 3600 * 1000),
        tags: { connect: skill.tags.map((name) => ({ id: tagByName.get(name).id })) },
      },
    });

    const usageCount = 3 + index;
    for (let usageIndex = 0; usageIndex < usageCount; usageIndex += 1) {
      await prisma.usageHistory.create({
        data: {
          skillId: created.id,
          accessedAt: new Date(now - (usageIndex + index) * 9 * 3600 * 1000),
          agentName: usageIndex % 2 === 0 ? "Claude 3.5" : "Hermes Librarian",
          accessMethod: usageIndex % 3 === 0 ? "recommendation" : "search",
        },
      });
    }
  }

  for (const workflow of workflows) {
    await prisma.workflow.create({ data: workflow });
  }

  for (const document of knowledgeDocuments) {
    await prisma.knowledgeDocument.create({
      data: {
        title: document.title,
        slug: document.slug,
        description: document.description,
        content: document.content,
        categoryId: categoryBySlug.get(document.categorySlug).id,
      },
    });
  }

  for (const recommendation of recommendations) {
    await prisma.recommendation.create({ data: recommendation });
  }
}

seed()
  .then(async () => {
    await prisma.$disconnect();
  })
  .catch(async (error) => {
    console.error(error);
    await prisma.$disconnect();
    process.exit(1);
  });
